///////////////////////////////////////////////
//
//  THIS VARIANT IS MADE TO SWITCH CURRENT DIRECTION WITH ALPHA WAVES
//  TUTORIAL LINK *HERE*
//
//
...
//define Direction
String direction_portName = "/dev/tty.usbmodem1421";  //modify this value to match usb port number used by your arduino 
Serial direction_serial;
int direction_baud = 115200; //baud rate from the Arduino
Direction direction;
...
void setup() {
  ...
  try {
    println("OpenBCI_GUI:  attempting to open serial port for Direction using name = " + direction_portName);
    println(Serial.list());
    direction_serial = new Serial(this,direction_portName,direction_baud); //open the com port
    direction_serial.clear(); // clear anything in the com port's buffer
   } catch (RuntimeException e){
    println("OpenBCI_GUI: *** ERROR ***: Could not open " + direction_portName);
  }
...
void initSystem(){
  ...
  direction = new Direction(direction_serial);
  eegProcessing_user = new EEG_Processing_User(nchan,openBCI.get_fs_Hz(),direction);
