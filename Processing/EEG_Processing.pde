...
class EEG_Processing_User {
  ...
  EEG_Processing_User(int NCHAN, float sample_rate_Hz, Direction _direction) {
    nchan = NCHAN;
...
//
  ...
  FFT[] fftData) {              //holds the FFT (frequency spectrum) of the latest data
    ...
    // mappedValue not used in the Direction class 
...
//              isDetected = false;
    ...
    if(isDetected){
      ...
      direction.switchDirection(); // send the ASCII command to the switch current direction 
    }
  ...
  boolean isDetected = false;
    ...
    int Ichan = 7-1;  //  the channel to process
    ...
    if (useClassfier_2DTraining) {
      ...
      // first check that any detectedPeaks are in the desired freqeuency range
      if ((detectedPeak[Ichan].freq_Hz >= processing_band_low_Hz[3-1]) && // <= 9Hz line 48
          (detectedPeak[Ichan].freq_Hz < processing_band_high_Hz[3-1])) { // < 12Hz line 50
        // then measure the peak against the background
        if (detectedPeak[Ichan].SNR_dB >= detection_thresh_dB) {// if it is pronounced
          detectedPeak[Ichan].threshold_dB = detection_thresh_dB;// on the fly threshold adjust (?)
          println("detectedPeak channel " + (Ichan) + " = " + detectedPeak[Ichan].SNR_dB);  // verbose debug
          int peak = int(detectedPeak[Ichan].SNR_dB) + 'A';  // map SNR.dB to ASCII Helicopter command

          detectedPeak[Ichan].isDetected = true;// flag that we got a winner
          isDetected = true;  // uncomment for verbose output
          if (isDetected) {
            //print some output
            println("EEG_Processing_User: alpha!, Chan " + (Ichan) + ", peak = " + detectedPeak[Ichan].rms_uV_perBin + " uV at "
...
//              isDetected = false;
      ...
      mappedValue += char(lastPeak);
      println("Detected Peak on chan " + peakChan + " Mapped Value: " + mappedValue);
      direction.switchDirection(); // send the ASCII command to switch current direction
    }
